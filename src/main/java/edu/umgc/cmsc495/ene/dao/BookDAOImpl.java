...
public class BookDAOImpl implements BookDAO {
	...
	public List<BookModel> searchBook(BookModel book) {
		...
		try (Connection connection = dataSource.getConnection();) {
			...
			StringBuilder query = new StringBuilder(
					"SELECT id, bookTitle, bookAuthor, bookCategory, isbn, publishDate, price, userId from BOOK where id is not null ");
	...
	public BookModel getBook(int bookId) {
		...
		String query = "SELECT id, bookTitle, bookAuthor, bookCategory, isbn, publishDate, price, userId from BOOK where id = "
				+ bookId;
		try (Connection connection = dataSource.getConnection();
				PreparedStatement ps = connection.prepareStatement(query)) {
	...
	public List<BookModel> searchBookByCriteria(String searchTerm) {
		...
		StringBuilder query = new StringBuilder(
				"SELECT id, bookTitle, bookAuthor, bookCategory, isbn, publishDate, price, userId from BOOK ");

		if (searchTerm != null) {
			query.append(" where bookTitle ILIKE '%" + toSearch + "%' OR bookAuthor ILIKE '%" + toSearch
					+ "%' OR isbn ILIKE '%" + toSearch + "%'");
		}
		try (Connection connection = dataSource.getConnection();
				PreparedStatement ps = connection.prepareStatement(query.toString())) {
